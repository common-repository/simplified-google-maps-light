...
if (!class_exists("Cetabo_SGMLResourceLoader")) {
    /**
     * Handles loading of CSS, JS and images files
     */
    class Cetabo_SGMLResourceLoader
    {
         ...
         * @return Cetabo_SGMLResourceLoader
         */
        public static function instance()
        {
            if (self::$instance == null) {
                self::$instance = new Cetabo_SGMLResourceLoader();
            }
        ...
        public function load_css_bundle($bundle)
        {
            foreach ($bundle as $bundle_entry) {
                if (is_array($bundle_entry)) {
                    $path = Cetabo_SGMLHelper::arr_get($bundle_entry, 0);
                    $is_local = Cetabo_SGMLHelper::arr_get($bundle_entry, 1);
                } else {
                    $path = $bundle_entry;
                    $is_local = true;
                }
        ...
        private function identifier($file_path)
        {
            ...
            $count = Cetabo_SGMLHelper::arr_get($this->resource_counter, $identifier, 0);
        ...
        {
            $name = str_replace(array('/', ':', '_', '.'), '-', Cetabo_SGMLRegistry::get('PLUGIN_SLUG') . '-' . $this->identifier($file_path));
            $file = Cetabo_SGMLRegistry::get('PLUGIN_DIR') . $this->clean($file_path);
            $url = ($is_local) ? Cetabo_SGMLRegistry::get('PLUGIN_URL') . $file_path : $file_path;
        ...
        {
            return Cetabo_SGMLRegistry::get('PLUGIN_URL') . $file;
        }
    }
}